...
export function EnhancedWeatherOverlay({
  weatherPoints,
  mapBounds,
}: EnhancedWeatherOverlayProps) {
  ...
  const getWeatherColor = (condition: string, intensity = 'medium') => {
    ...
    const intensityModifier =
      intensity === 'high' ? '/90' : intensity === 'low' ? '/60' : '/80';
            ...
            <div className='relative'>
              ...
              <div
                className={`${getWeatherColor(
                  weather.condition,
                  intensity,
                )} text-white p-2 rounded-full shadow-lg backdrop-blur-sm border border-white/20`}
              >
              ...
              <div className='absolute top-full left-1/2 transform -translate-x-1/2 mt-2 bg-slate-900/95 text-white text-xs px-2 py-1 rounded whitespace-nowrap opacity-0 hover:opacity-100 transition-opacity pointer-events-auto'>
                ...
                <div>
                  Wind: {weather.windSpeed}kt{' '}
                  {weather.windDirection.toUpperCase()}
                </div>
                {weather.temperature && (
